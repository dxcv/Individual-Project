...
@author: Beryl
"""
#### IN record, accuracy A slightly different because of the dropout in the classifiers
...
#from tagger_filter2 import Tagger
from tagger_orth import Tagger
from game_ner_new2 import Env
from sklearn.metrics import f1_score
from collections import Counter
import csv
from joblib import Parallel, delayed
from apply_filter import bald_sampling, VarRatio
...
fndict = {"rs": lambda feature_now, model_selected, budget, train_data: random_sampling(feature_now, model_selected, budget, train_data), 
          ...
          "bald": lambda feature_now, model_selected, budget, train_data: bald_sampling(feature_now, model_selected, budget, train_data),
          "var": lambda feature_now, model_selected, budget, train_data: VarRatio(feature_now, model_selected, budget, train_data)}
...
def play_ner(feature_now, model_ver, poly, cvit, logit, fcls, method, n_states, al_method):
    ...
    while ID_student < len(test_child):
        ...
        queried_set_xx = np.load('Filter_again_07_29_exp_{0}/test_{1}/pool_x_budget_{2}_ID_{3}.npy'.format(23, cvit, BUDGET, ID_student))                
        indexs = [0, 257, 327, 354, 378]
        pool_x = []
        for i in range(4):
            pool_x.append(queried_set_xx[:,:,indexs[i]:indexs[i+1]])
        pool_y = np.load('Filter_again_07_29_exp_{0}/test_{1}/pool_y_budget_{2}_ID_{3}.npy'.format(23, cvit, BUDGET, ID_student))
...
def write_test_csv(test_game, model, student_ID, model_ver, cvit, al_method):
    csv_name = 'Filpool_baye_exp_{0}/test_{1}/results/{2}_'.format(EXPNUM, cvit, al_method)+str(model_ver)+'.csv'
    if not os.path.exists('Filpool_baye_exp_{0}/test_{1}/results'.format(EXPNUM, cvit) + os.sep):
            os.makedirs('Filpool_baye_exp_{0}/test_{1}/results'.format(EXPNUM, cvit) + os.sep)
