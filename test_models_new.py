...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 26 07:17:16 2019
...
#LOGIT = True
LOGIT = False
FEATURES = [[1,2,3,4]] # WHAT ABOUT THE OTHER FEATURE COMBINATIONS?
...
def initialise_game(model, budget, niter, feature_number, student, cvit, method):


    
    train_x, train_y = helper.test_data_auto(model, feature_number, student)
...
def play_ner(feature_now, model_ver, poly, cvit, method):
    actions = 2
    global BUDGET
    ...
    with tf.name_scope(model_ver):
        model = Tagger(model_file=model_ver+'/feature_5',
                            n_input=FEATURE_SHAPE[0][0],n_steps=FEATURE_SHAPE[0][1],feature_number=5, epochs=NITER, expnum = EXPNUM,cvit=cvit)
    ...
    test_game = initialise_game(model_selected,BUDGET,NITER,FEATURE, test_child[ID_student], cvit, method)
    while ID_student < len(test_child):
        observation = test_game.get_frame(model_selected)
        action = robot.test_get_action(model_ver,observation)
        ...
        if test_game.count_action1 == BUDGET or test_game.current_frame == len(test_game.train_y_all)-1:
            if len(model_selected) == 1:
                model_selected[0].train_mode_B(test_game.queried_set_x, test_game.queried_set_y,5)
            else:
...
def write_test_csv(test_game, model, student_ID, model_ver, cvit):
    ...
    for i in range(len(model)):
        accuracy_test_A.append(float("%.3f" % model[i].test(test_game.test_x_all,test_game.test_y_all)))
        f1_test_A.append(float("%.3f" % model[i].get_f1_score(test_game.test_x_all,test_game.test_y_all)))
        

        
        accuracy_test_B.append(float("%.3f" % model[i].test_B(test_game.test_x_all,test_game.test_y_all)))
        f1_test_B.append(float("%.3f" % model[i].get_f1_score_B(test_game.test_x_all,test_game.test_y_all)))
        
        
        if len(model) ==1:
            print(np.array(test_game.test_x_all[i]).shape)
            conf_test_A.append(float("%.3f" % np.mean(model[i].get_confidence(test_game.test_x_all))))
            conf_test_B.append(float("%.3f" % np.mean(model[i].get_confidence_B(test_game.test_x_all))))
            predd_test_A.append(model[i].get_predictions(test_game.test_x_all))
            predd_test_B.append(model[i].get_predictions_B(test_game.test_x_all))
        else:
            print(np.array(test_game.test_x_all[i]).shape)
    ...
    predd_test_A = np.array(predd_test_A).reshape(-1,)
    predd_test_B = np.array(predd_test_B).reshape(-1,)


    y_multi_test_A, y_multi_test_B = predd_test_A, predd_test_B

    accuracy_multi_test_A = sum(np.equal(test_game.train_y_all, y_multi_test_A))/len(test_game.train_y_all)
    f1_multi_test_A = f1_score(test_game.train_y_all, y_multi_test_A, average='macro')
    accuracy_multi_test_B = sum(np.equal(test_game.test_y_all, y_multi_test_B))/len(test_game.test_y_all)
    f1_multi_test_B = f1_score(test_game.test_y_all, y_multi_test_B, average='macro')
    ...
    writer.writerow({"student_ID": student_ID, 
                     ...
                     "f1_test_A": f1_test_A,
                     "accuracy_majority_test_A": float("%.3f" % accuracy_multi_test_A),
                     "f1_majority_test_A": float("%.3f" % f1_multi_test_A),
                     "conf_test_A": conf_test_A,
                     "count_0_B": test_game.count_action0,
                     "count_1_B": test_game.count_action1,
                     "accuracy_test_B": accuracy_test_B,
                     "f1_test_B": f1_test_B,
                     "accuracy_majority_test_B": float("%.3f" % accuracy_multi_test_B),
                     "f1_majority_test_B": float("%.3f" % f1_multi_test_B),
...
# =============================================================================
                ...
                play_ner(FEATURE, MODEL_VER, POLY,cvit, method)
                tf.reset_default_graph()
...
if __name__ == '__main__':
    ...
    Parallel(n_jobs=num_cores)(delayed(main)(i) for i in itertools.product(BUDGETS,FEATURES,cvits))
    #main([5,FEATURES[0],0])                          