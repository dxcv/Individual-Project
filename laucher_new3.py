...
from robotDQN_new4 import RobotLSTMQ
import numpy as np
import helper
...
## 23 without softmax
## 24 with softmax
## 25, content1 without softmax, only budget 20, 50, 100
## 31: content 0, only softmax, with dropout, with orthogonal intializer
EXPNUM = 31 #args.exp_num[0]
...
# changable variables
BUDGETS = [50, 100]
# when n_states = 0, average out the raw contents
NSTATES = [0]
#NSTATES = [0]
...
def play_ner(feature_now, model_ver, poly, niter, logit, fcls, method, n_states):
    actions = 2
    global BUDGET
    ...
    for NITER in NITERS:
        for CONTENT in CONTENTS:
            for CUM in CUMS:
                ...
                robot = play_ner(FEATURE, MODEL_VER, POLY, NITER, LOGIT, FCLS, method, n_states)
                tf.reset_default_graph()


if __name__ == '__main__':
    #main([5,[1,2,3,4],'maj','d1qn'])
    #main([5,[0],'maj','d1qn', 0])
