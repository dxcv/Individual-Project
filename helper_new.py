...
def load_traindata(feature,n_input,seed):
    ...
    dataset_x = np.array(dataset_x).reshape(-1,10, n_input)
...
def load_testdata(feature, n_input, seed):
...
def write_csv_game(env, model):
    ...
    for i in range(len(model)):
        ...
        accuracy_test.append(float("%.3f" % model[i].test(env.test_x_all, env.test_y_all)))
        conf_all_train.append(model[i].get_confidence(env.train_x_all))
        conf_train.append(float("%.3f" % np.mean(model[i].get_confidence(env.train_x_all))))
        conf_all_test.append(model[i].get_confidence(env.test_x_all))
        conf_test.append(float("%.3f" % np.mean(model[i].get_confidence(env.test_x_all))))
        f1_train.append(float("%.3f" % model[i].get_f1_score(env.train_x_all, env.train_y_all)))
        f1_test.append(float("%.3f" % model[i].get_f1_score(env.test_x_all, env.test_y_all)))
        predd_train.append(model[i].get_predictions(env.train_x_all))
        predd_test.append(model[i].get_predictions(env.test_x_all))

    predd_train = np.array(predd_train).reshape(-1,)
    predd_test = np.array(predd_test).reshape(-1,)
    conf_all_train = np.array(conf_all_train).T.tolist()
    conf_all_test = np.array(conf_all_test).T.tolist()

    y_multi_train, y_multi_test = predd_train, predd_test
...
def test_data_auto(tagger, features_selected, student):
