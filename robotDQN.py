...
class RobotLSTMQ:
    ...
    def __init__(self, actions, features, content, poly, logit, ntype, expnum):
    ...
    def train_qnetwork(self):
        ...
        for item in next_state_batch:
            sent, confidence, predictions, logits = item
    ...
    def get_action(self, observation):
        ...
        self.current_state = observation
        sent, confidence, predictions, logits = self.current_state
...
class q_network():
    ...
    def __init__(self, robot, scope):
        ...
        if self.content:
            ...
            if self.logit:
                def lstm2(x, weights, biases, i):
                    x = tf.unstack(x, self.feature_shape[i][1], 1)
                    with tf.variable_scope(scope+'_'+str(i)+'_lstml1'):
                        lstm_cell = rnn.BasicLSTMCell(self.n_hidden, forget_bias=1.0)
                    with tf.variable_scope(scope+'_'+str(i)+'_lstml2'):
                ...
                self.xlogits = tf.placeholder(tf.float32, [None, self.n_features, 10, 64], name="x_logits")
                self.weight_proj_l = {'out': tf.Variable(tf.random_normal([self.n_hidden, 64]))}
                self.biases_proj_l = {'out': tf.Variable(tf.random_normal([64]))}
            # confidence is a scalar
            #self.enc_total = tf.concat((self.enc_s, self.state_confidence, self.predictions), axis=1)
                self.final_logits = []
                for i in range(self.n_features):
                    #print('shape logit: ', tf.shape(self.xlogits[:,i]))
                    final_l = lstm2(self.xlogits[:,i], self.weight_proj_l, self.biases_proj_l,i) #should have shape of 32
                    self.final_logits.append(final_l)
                self.final_logits = tf.stack(self.final_logits, axis=2)
                print('shape enc_s', self.final_logits.get_shape())
                self.final_logits = tf.reshape(self.final_logits, [-1,64*self.n_features])
            ...
            for i in range(self.n_features):
                final_l = lstm2(self.xlogits[:,i], self.weight_proj_l, self.biases_proj_l,i) #should have shape of 32
                self.final_logits.append(final_l)
            self.enc_total = tf.stack(self.final_logits, axis=2)
            self.enc_total = tf.reshape(self.enc_total, [-1,64*self.n_features])
