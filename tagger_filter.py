...
class Tagger(object):
    ...
    def train_mode_B(self, data_x, data_y, feature_number, mode = 'B'):
        ...
        if not os.path.exists(self.header_test + os.sep + 'test_{1}/{0}/'.format(mode, self.cvit) +self.model_file):
            os.makedirs(self.header_test + os.sep + 'test_{1}/{0}/'.format(mode, self.cvit) + self.model_file)
        ...
        for i in range(self.epochs):
            ...
            if (i+1) % self.save_step == 0:
                self.saver.save(self.sess, self.header_test + os.sep  + 'test_{1}/{0}/'.format(mode, self.cvit) + self.model_file + os.sep + 'model.ckpt', i+1)
    ...
    def get_predictions_B(self, x, mode='B'):
        ...
        ckpt = tf.train.get_checkpoint_state(self.header_test + os.sep + 'test_{0}/{1}/'.format(self.cvit, mode) +self.model_file)
        self.saver.restore(self.sess, ckpt.model_checkpoint_path)  # no need to restore since we use the last one
