...
class Tagger(object):
    ...
    def __init__(self, model_file, n_steps, n_input, feature_number, training = True, epochs = 10, expnum = 0, cvit=None):
        ...
        def lstm(x, weights, biases):
            ...
            with tf.variable_scope('tagger1_feature_{0}'.format(self.feature_number), initializer=tf.orthogonal_initializer()):
                lstm_cell = rnn.BasicLSTMCell(self.n_hidden, forget_bias=1.0)
            with tf.variable_scope('tagger2_feature_{0}'.format(self.feature_number), initializer=tf.orthogonal_initializer()):
