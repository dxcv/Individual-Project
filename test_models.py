...
def initialise_game(model, budget, niter, feature_number, student, cvit):
	...
	game = Env(story, test, dev, budget, MODEL_VER, model,feature_number, CUM, EXPNUM, 0, 'd1qn')
	return game

		
def play_ner(feature_now, model_ver, poly, cvit):
	actions = 2
	global BUDGET
	tf.reset_default_graph()
	if AGENT == "LSTMQ":
		robot = RobotLSTMQ(actions, FEATURE, content = CONTENT, poly = poly, logit=True, ntype = NTYPE, expnum = EXPNUM)
	else:
	...
	while ID_student < len(test_child):
		...
		################ test mode A################
		if test_game.count_action1 == BUDGET_TEST or test_game.current_frame == len(test_game.train_y_all)-1:
		...
		for CONTENT in CONTENTS:
			for CUM in CUMS:
				...
				if CONTENT: 
					...
					play_ner(FEATURE, MODEL_VER, POLY,cvit)
					tf.reset_default_graph()
				else:
					#for poly in POLYS:
					POLY = False
